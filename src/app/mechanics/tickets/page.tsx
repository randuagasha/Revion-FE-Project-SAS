  ...
  useEffect(() => {
    const fetchTickets = async () => {
      try {
        ...
        console.error("Failed to fetch mechanic tickets:", error);

        setTickets([]);
        setPageError(
          "Failed to load tickets. Make sure the backend allows mechanic to access related tickets.",
        );
      } finally {
        setLoading(false);
      }
    ...
    <div className="space-y-6">
        ...
        <div className="relative z-10 flex flex-col justify-between gap-5 lg:flex-row lg:items-center">
          <div>
            ...
            <h1 className="max-w-2xl text-2xl font-bold leading-tight text-white md:text-3xl">
              Handle customer tickets related to your assigned jobs.
            </h1>

            <p className="mt-3 max-w-2xl text-sm leading-6 text-muted-foreground">
              View and respond to customer support tickets connected to vehicles
              you have handled or are currently handling.
            </p>
          </div>
        </div>
      ...
      <section className="rounded-3xl border border-border bg-card/40 p-5 backdrop-blur-xl">
        <div className="mb-5 flex flex-col justify-between gap-4 lg:flex-row lg:items-center">
          <div>
            <h2 className="text-xl font-bold text-white">Ticket List</h2>

            <p className="mt-1 text-sm text-muted-foreground">
              Search and manage tickets related to your mechanic jobs.
            </p>
          </div>
        ...
        {loading ? (
          ...
          <div className="flex min-h-72 flex-col items-center justify-center rounded-2xl border border-dashed border-border text-center">
            ...
            <p className="mt-2 max-w-md text-sm leading-6 text-muted-foreground">
              Customer tickets related to your handled vehicles will appear
              here.
            </p>
          </div>
        ) : (
          <div className="grid gap-4 xl:grid-cols-2">
            {filteredTickets.map((ticket) => (
              <Link
                key={ticket.id}
                href={`/mechanics/tickets/${ticket.id}`}
